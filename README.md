...
这是一个无需后台即可部署的纯静态导航网站。优化版已重新接入完整品牌素材，并为桌面、手机、卡片、钱包和图标场景生成经过压缩的 WebP 图片。
...
- 当前公共视觉：`assets/css/site-v2.css`
- 公共菜单和页尾：`assets/js/components.js`
- 菜单、复制和外链交互：`assets/js/site.js`
- 优化图片：`assets/images/`
- 图片制作尺寸：`IMAGE-SPECS.md`
- 页面条目和外链：对应 HTML 文件

## 设计说明

保留原有黑金风格和品牌图片，但移除了虚假的实时延迟、无法验证的安全承诺及无功能占位链接。外部链接会显示跳转标识并使用安全的窗口隔离属性。上传到 GitHub 时必须连同整个 `assets` 文件夹一起上传。
